# 快速上手（完整链路：WordPress HTTPS + LiteLLM + HTTPS 子域名 + 插件）
...
> 注意：部署脚本 `scripts/deploy-full.sh` **只负责 LiteLLM（Docker）**，不负责 Nginx/证书/WordPress；这些步骤在本章与 `05-WordPress-集成（企业级要点）.md` 中手工完成。
...
- WordPress 后台域名：`https://wp.example.com`
- 计划的 LiteLLM 子域名：`https://litellm.example.com`
- 证书：通配符证书 `*.example.com`（✅覆盖 `litellm.example.com`）

你需要确认：

- **DNS**：`litellm.example.com` 的 A 记录指向本服务器公网 IP
...
- `litellm.yourcompany.com` → `litellm.example.com`
- `https://your-wordpress-domain.com` → `https://wp.example.com`
...
- LiteLLM 健康：`curl -fsS http://127.0.0.1:24157/health/liveliness`
- 反代 UI：浏览器打开 `https://litellm.example.com/ui/`
