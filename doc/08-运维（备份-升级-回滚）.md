...
- 观测栈配置：`/opt/litellm-server/observability/`
...
# 2) 备份配置与 .env（注意权限）
sudo tar -C /opt/litellm-server -czf "/opt/backup/litellm_config_${TS}.tar.gz" config observability docker-compose.yml docker-compose.observability.yml .env
...
4) **验收**：`/health`、核心 `/v1/...`、WordPress iframe `/ui`、Prometheus 抓取、关键告警状态  
