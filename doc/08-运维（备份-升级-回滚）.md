...
# 1) 修改 .env 中 LITELLM_IMAGE（从 v1.80.11-stable -> v1.80.xx-stable）
sudo vi .env
...
1) **回退镜像版本**（把 `.env` 中 `LITELLM_IMAGE` 改回升级前的版本，例如 `v1.80.11-stable`）  
2) **重建服务**：`docker compose up -d`  
3) **必要时恢复数据库**（如果升级产生不兼容变更）  
4) **验收**：`/health/liveliness`、核心 `/v1/...`、WordPress iframe `/ui`  
